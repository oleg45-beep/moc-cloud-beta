...
def init_db():
    ...
    try:
        ...
        files_columns = [col[1] for col in c.fetchall()]
        ...
        for col_name, col_type in columns_to_add:
            if col_name not in files_columns:
                try:
        ...
        if 'is_trusted_for_recovery' not in friends_columns:
            try:
                c.execute("ALTER TABLE friends ADD COLUMN is_trusted_for_recovery INTEGER DEFAULT 0")
                print("✅ Added column 'is_trusted_for_recovery' to friends table")
            except Exception as e:
                print(f"⚠️ Could not add is_trusted_for_recovery: {e}")
        
        if 'trust_level' not in friends_columns:
            try:
                c.execute("ALTER TABLE friends ADD COLUMN trust_level INTEGER DEFAULT 1")
                print("✅ Added column 'trust_level' to friends table")
            except Exception as e:
                print(f"⚠️ Could not add trust_level: {e}")
        
        c.execute("PRAGMA table_info(user_keys)")
        user_keys_columns = [col[1] for col in c.fetchall()]
        
        if 'key_setup_at' not in user_keys_columns:
            try:
                c.execute("ALTER TABLE user_keys ADD COLUMN key_setup_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP")
                print("✅ Added column 'key_setup_at' to user_keys table")
            except Exception as e:
                print(f"⚠️ Could not add key_setup_at: {e}")
        
        c.execute("PRAGMA table_info(messages)")
        messages_columns = [col[1] for col in c.fetchall()]
        
        if 'is_notification' not in messages_columns:
            try:
                c.execute("ALTER TABLE messages ADD COLUMN is_notification INTEGER DEFAULT 0")
                print("✅ Added column 'is_notification' to messages table")
            except Exception as e:
                print(f"⚠️ Could not add is_notification: {e}")
    ...
    try:
        c.execute("SELECT id FROM users WHERE username = 'support'")
        if not c.fetchone():
...
if __name__ == '__main__':
    print("=" * 60)
    print("🚀 MOC Secure Cloud Server")
    print("=" * 60)
    print(f"📁 Upload folder: {UPLOAD_FOLDER}")
    print(f"🔐 Encrypted folder: {ENCRYPTED_FOLDER}")
    print(f"💾 Database: {DB_NAME}")
    print(f"🌍 URL: http://localhost:5000")
    print(f"🕒 Timezone: Moscow (UTC+3)")
    print("=" * 60)
